...
import { Search, Filter, Users, Clock, MapPin, TrendingUp, Shield, Heart, Target, AlertTriangle } from 'lucide-react';
...
const RequestsPage: React.FC = () => {
  ...
  const getCategoryColor = (category: string) => {
    const colors = {
      ...
      medical: 'bg-blue-500/20 text-blue-400 border border-blue-500/30',
      housing: 'bg-purple-500/20 text-purple-400 border border-purple-500/30',
    ...
    return colors[category as keyof typeof colors] || 'bg-gray-500/20 text-gray-400 border border-gray-500/30';
  };
          ...
          >
            <div className="flex items-center justify-center text-amber-400">
              ...
              <span className="font-medium">
                Insufficient Balance ({balance} ETH) - Add funds to your wallet to make donations
              </span>
            </div>
          </motion.div>
        )}
        ...
        >
          <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl p-6 border border-slate-700/50">
            ...
            <div className="text-2xl font-bold text-white mb-1">
              {requests.reduce((sum, r) => sum + parseFloat(r.raised), 0).toFixed(2)} ETH
            </div>
          ...
          <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl p-6 border border-slate-700/50">
            ...
            <div className="text-2xl font-bold text-white mb-1">
              {requests.reduce((sum, r) => sum + r.contributors, 0)}
            </div>
            <div className="text-sm text-slate-300">Total contributors</div>
          </div>
            ...
            >
              ...
              <div className="flex items-center justify-between">
                ...
                <Link
                  ...
                  className="px-4 py-2 bg-gradient-to-r from-cyan-500 to-blue-600 text-white rounded-lg hover:from-cyan-600 hover:to-blue-700 transition-all duration-200 font-medium text-sm group-hover:shadow-lg group-hover:shadow-cyan-500/25"
                >
                  {isConnected && parseFloat(balance) >= 0.01 ? 'Donate Now' : 'View Details'}
                </Link>
              </div>
            </motion.div>
          ))}
